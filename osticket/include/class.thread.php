...
function getAttachmentsLinks($file='attachment.php', $target='', $separator=' ') {
    ...
    function getAttachmentFile() {
        ...
        foreach($this->getAttachments() as $attachment ) {
            ...
            /* The hash can be changed  but must match validation in @file */
//            $hash=md5($attachment['file_id'].session_id().$attachment['file_hash']);
              $hash=md5($attachment['file_id'].$attachment['file_hash']);

            $size = '';
            if($attachment['size'])
                $infoFile['namefile']=Format::htmlchars($attachment['name']);
                $infoFile['size']=Format::file_size($attachment['size']);
                $infoFile['attach_id']=$attachment['attach_id'];
//                $infoFile['links']="http:/".$_SERVER["SERVER_NAME"]."/attachment.php?id=".$attachment['attach_id']."&h=".$hash;
