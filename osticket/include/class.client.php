...
abstract class TicketUser {
    ...
    function __call($name, $args) {
        ...
        //$token_Send=$this->jos_encode($ticket->getId(), $ticket->getEmail());
        //$domainclient='http://wiserobotshop.wiserobot';
        //$tag =  substr($name, 3);
        ...
        switch (strtolower($tag)) {
            ...
            //case 'ticket_link':
               //if($ticket->getSource()=='Email'){
                  // return sprintf('%s/osticket/index/view/token/%s',$domainclient,$token_Send);
                   
               //}else return sprintf('%s/osticket/index/view',$domainclient);
    ...
    //write by vtuong
    function jos_encode($number=0,$str=""){
        $str=$number.'['.$str;
        $str = strtoupper($str);
        $characters=".0123456789@ABCDEFGHIJKLMNOPQRSTUVWXYZ[";
        $c1=$characters[rand(0, strlen($characters))];
        $c2=$c1;
        while($c1==$c2) { $c2=$characters[rand(0, strlen($characters))];}
        $key=$c1;$val=$c2;
        //echo gettype(ord($c1));
        if($c1>$c2){ $val=$c1; $key=$c2;}
        // $kc=abs($c1-$c2);
        $posi_val=strpos($characters,$val);
        $posi_key=strpos($characters,$key);
        
        $strUse="";
        
        for($i=0;$i<$posi_key;$i++){
            $strUse.=$val;
            if($val=='[') $val='.';
            else if($val=='.') $val='0';
            else if($val=='9') $val='@';
            else $val=chr(ord($val)+1);
        }
        for($i=$posi_key;$i<strlen($characters);$i++){
            $strUse.=$val;
            if($val=='[') $val='.';
            else if($val=='.') $val='0';
            else if($val=='9') $val='@';
        ...
        for($i=0;$i<strlen($str);$i++){
            $p=strpos($characters, $str[$i]);
            //echo $str[$i];        
            $mh.=substr($strUse,$p,1);
        }
        $mh.=$val;
        return $mh;
    }
